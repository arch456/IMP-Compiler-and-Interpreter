...
def get_label_from_def(instr_tuple):
    ...
    instr_name = instr_tuple[0]
    label = instr_name[0:-1] # skip the ending colon
    return label
...
def relative_instr(offset, curr_ix, instr_stream):
    ...
    if curr_ix + offset > len(instr_stream)-1:
        raise ValueError("relative isntruction index out of bounds: " + str(curr_ix+ffset))
    elif curr_ix + offset < 0:
        raise ValueError("relative isntruction index out of bounds" + str(curr_ix+offset))
    else:
